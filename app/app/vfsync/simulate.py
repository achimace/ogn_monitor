...
Emits exactly the ``event:{slug}`` messages the tracking worker would
publish for one flight (takeoff, launch_type_detected, touch_and_go,
landing, landing_final) so VF-Sync can be exercised end to end against
the VF mock without real flying. Uses the production RedisWriter, so the
payload format is the real one. Never run this against a tenant whose
VF-Sync points at the real Vereinsflieger.
"""
    ...
    try:
        for i, (etype, flight, message) in enumerate(events):
            await writer.publish_event(args.slug, etype, flight.flarm_id,
                                       data=flight.to_redis_dict(), message=message)
